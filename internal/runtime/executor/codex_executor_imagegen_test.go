...
func TestEnsureImageGenerationToolForSource_SkipsClaudeSource(t *testing.T) {
	body := []byte(`{"model":"gpt-5.5","input":"hi"}`)
	result := ensureImageGenerationToolForSource(body, "gpt-5.5", sdktranslator.FromString("claude"), nil)

	if string(result) != string(body) {
		t.Fatalf("expected Claude source body to be unchanged, got %s", string(result))
	}
	if gjson.GetBytes(result, "tools").Exists() {
