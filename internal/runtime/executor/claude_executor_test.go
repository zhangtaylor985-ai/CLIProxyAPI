...
func TestApplyClaudeHeaders_UsesConfiguredBaselineFingerprint(t *testing.T) {
	...
	applyClaudeHeaders(req, auth, "key-baseline", false, nil, cfg, false)
...
func TestApplyClaudeHeaders_StripsOpus1MSignalsWhenRequested(t *testing.T) {
	resetClaudeDeviceProfileCache()

	auth := &cliproxyauth.Auth{
		ID: "auth-opus-base-only",
		Attributes: map[string]string{
			"api_key": "key-opus-base-only",
		},
	}
	req := newClaudeHeaderTestRequest(t, http.Header{
		"Anthropic-Beta":  []string{"foo, context-1m-2025-08-07, bar"},
	...
	applyClaudeHeaders(
		...
		"key-opus-base-only",
		false,
		[]string{"context-1m-2025-08-07", "other-beta"},
		&config.Config{},
		true,
	)

	if got := req.Header.Get("Anthropic-Beta"); strings.Contains(got, "context-1m-2025-08-07") {
		t.Fatalf("Anthropic-Beta should not contain 1M beta, got %q", got)
	}
	if got := req.Header.Get("Anthropic-Beta"); !strings.Contains(got, "other-beta") {
		t.Fatalf("Anthropic-Beta should preserve non-1M betas, got %q", got)
	}
}
...
func TestApplyClaudeHeaders_TracksHighestClaudeCLIFingerprint(t *testing.T) {
	...
	applyClaudeHeaders(firstReq, auth, "key-upgrade", false, nil, cfg, false)
	...
	applyClaudeHeaders(thirdPartyReq, auth, "key-upgrade", false, nil, cfg, false)
	...
	applyClaudeHeaders(higherReq, auth, "key-upgrade", false, nil, cfg, false)
	...
	applyClaudeHeaders(lowerReq, auth, "key-upgrade", false, nil, cfg, false)
...
func TestApplyClaudeHeaders_DoesNotDowngradeConfiguredBaselineOnFirstClaudeClient(t *testing.T) {
	...
	applyClaudeHeaders(olderClaudeReq, auth, "key-baseline-floor", false, nil, cfg, false)
	...
	applyClaudeHeaders(newerClaudeReq, auth, "key-baseline-floor", false, nil, cfg, false)
...
func TestApplyClaudeHeaders_UpgradesCachedSoftwareFingerprintWhenBaselineAdvances(t *testing.T) {
	...
	applyClaudeHeaders(officialReq, auth, "key-baseline-reload", false, nil, oldCfg, false)
	...
	applyClaudeHeaders(thirdPartyReq, auth, "key-baseline-reload", false, nil, newCfg, false)
...
func TestApplyClaudeHeaders_LearnsOfficialFingerprintAfterCustomBaselineFallback(t *testing.T) {
	...
	applyClaudeHeaders(thirdPartyReq, auth, "key-custom-baseline-learning", false, nil, cfg, false)
	...
	applyClaudeHeaders(officialReq, auth, "key-custom-baseline-learning", false, nil, cfg, false)
	...
	applyClaudeHeaders(postLearningThirdPartyReq, auth, "key-custom-baseline-learning", false, nil, cfg, false)
...
func TestApplyClaudeHeaders_ThirdPartyBaselineThenOfficialUpgradeKeepsPinnedPlatform(t *testing.T) {
	...
	applyClaudeHeaders(thirdPartyReq, auth, "key-third-party-then-official", false, nil, cfg, false)
	...
	applyClaudeHeaders(officialReq, auth, "key-third-party-then-official", false, nil, cfg, false)
	assertClaudeFingerprint(t, officialReq.Header, "claude-cli/2.1.77 (external, cli)", "0.87.0", "v24.8.0", "MacOS", "arm64")
}
...
func TestApplyClaudeHeaders_DisableDeviceProfileStabilization(t *testing.T) {
	...
	applyClaudeHeaders(firstReq, auth, "key-disable-stability", false, nil, cfg, false)
	...
	applyClaudeHeaders(thirdPartyReq, auth, "key-disable-stability", false, nil, cfg, false)
	...
	applyClaudeHeaders(lowerReq, auth, "key-disable-stability", false, nil, cfg, false)
...
func TestApplyClaudeHeaders_LegacyModePreservesConfiguredUserAgentOverrideForClaudeClients(t *testing.T) {
	...
	applyClaudeHeaders(req, auth, "key-legacy-ua-override", false, nil, cfg, false)
...
func TestApplyClaudeHeaders_LegacyModeFallsBackToRuntimeOSArchWhenMissing(t *testing.T) {
	...
	applyClaudeHeaders(req, auth, "key-legacy-runtime-os-arch", false, nil, cfg, false)
...
func TestApplyClaudeHeaders_UnsetStabilizationAlsoUsesLegacyRuntimeOSArchFallback(t *testing.T) {
	...
	applyClaudeHeaders(req, auth, "key-unset-runtime-os-arch", false, nil, cfg, false)
