...
- 2026-05-14 已上线 worker 自定义镜像 `cliproxy-api-worker:09528a02`，构建上下文在 `/root/cliproxy-workers/custom-image-09528a02/`，并保留 `source.patch`。该版本在 `ed0d6d74` 滚动 prompt cache 基础上，支持消费主程序转发的 `prompt_cache_key`，避免不同客户/会话在 worker 内退化成共享同一个 API key 级缓存命名空间。
