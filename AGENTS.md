...
- worker VPS 当前运行 8 个 Docker 容器：`cliproxy-worker01` 到 `cliproxy-worker08`；每个容器只挂载 1 个 Codex auth file、1 份独立 `config.yaml`、1 个独立住宅代理。生产主程序只接入健康 worker，未接入的容器可保留用于后续修复和单独验证。
- 2026-05-14 起 worker 源码由 GitHub 仓库 `https://github.com/zhangtaylor985-ai/CLIProxyAPI-pure.git` 管理；本地源目录为 `/Users/taylor/sdk/CLIProxyAPI-pure`，worker VPS clone 目录为 `/root/cliproxy-workers/CLIProxyAPI-pure`。上游官方仓库保留为 `upstream` remote。当前 worker 自定义镜像至少包含 forwarded `prompt_cache_key` 稳定修复和管理接口 runtime 状态字段；具体运行镜像以 worker VPS `docker ps --filter name=cliproxy-worker` 为准。
- 当前缓存策略：主程序负责 session-affinity，把同一会话尽量固定到同一个 worker。2026-05-14 起，主程序对 `codex-worker*` 的 Claude `/v1/messages` 流量默认直接转发到 worker 的 `/v1/messages`，不再先翻译成 OpenAI-compatible `/chat/completions`，避免形成“主程序 Claude->OpenAI + worker OpenAI->Codex”的二次翻译链路；worker 按原项目 Claude->Codex 路径自行生成和维护 prompt cache scope。旧的主程序会话级 `prompt_cache_key` 注入只作为回退路径保留：当 worker 显式配置 `codex_worker_claude_direct=false` / `worker_claude_direct=false` 或非 Claude 来源时，主程序才对 OpenAI-compatible 请求注入不含明文身份的会话级 `prompt_cache_key`。worker 内部仍按 `prompt_cache_key + base model + auth isolation key` 或 `metadata.user_id + base model + auth isolation key` 隔离缓存；OpenAI-compatible 回退链路、Claude 直连链路和 Codex WebSocket 链路都会在看到上游 usage 后观察真实 `cache_read_input_tokens` / `cached_tokens`。如果总输入增长但 cached tokens 卡在固定值，不要反复换 key；worker 失败切换时接受缓存损失，由新 worker 重新建立自己的缓存层级。
- worker 目录统一在 `/root/cliproxy-workers/`；注册表为 `/root/cliproxy-workers/worker_registry.tsv`，包含敏感 API key，不能外传或写入仓库。
- worker 容器本机端口为 `18317-18324`；主程序不直接通过公网访问这些端口，而是通过 worker VPS 主动建立的 SSH 反向隧道访问主程序本机 `127.0.0.1:18317-18324`。
- worker VPS 的反向隧道 systemd unit：`cliproxy-workers-reverse-tunnel.service`；端口访问限制 unit：`cliproxy-workers-firewall.service`。
- 主程序 `config.yaml` 中的 worker provider 块用 `# BEGIN CODEX WORKER PROVIDERS` / `# END CODEX WORKER PROVIDERS` 标记；截至 2026-05-14 09:30 UTC，生产仅接入健康的 `worker03/04/06` 共 3 个 OpenAI-compatible provider，provider 名称采用 `codex-workerNN-shortlabel`，方便在主程序管理端按 provider 查看每个 auth file 的用量。`worker08` 因连续出现上游 `context canceled` / HTTP2 `INTERNAL_ERROR` 类 500，已先从主程序路由池摘除，待单独排查后再恢复。
...
- 2026-05-14 worker 稳定性策略更新：生产确认 `worker02/05/07` 为 `usage_limit_reached` 额度耗尽，`worker01` 仍不适合承载主流量，`worker08` 当前链路不稳；当前从主程序 provider 池摘除 `worker01/02/05/07/08`，仅保留 `worker03/04/06`。主程序必须区分额度耗尽和首包空流：额度耗尽按整 worker/auth 长冷却；`empty_stream before first payload` 按 auth-level transient health 计数，第一次只记录并允许同 worker 短重试，连续失败达到阈值才整 worker 短冷却；成功请求清空连续空流计数。会话粘性只在绑定 worker 仍可用时复用，绑定 worker 不可用时必须 reselect 到可用 worker。
- 2026-05-14 第二阶段 worker 稳定性修复：OpenAI-compatible worker 有时会以 HTTP 200 建立 SSE 后，在首个 `data:` 帧返回 `{"error":...}` / `{"type":"error","error":...}`。主程序必须把这类 SSE 错误帧转换为 executor `StreamChunk.Err`，不能当成普通流内容透传；否则 auth manager 无法触发请求内 failover、auth-level transient health 和 worker 冷却。普通 `chat.completion.chunk` 与 `[DONE]` 不应被误判。生产 auth id 形如 `openai-compatibility:codex-worker03-...`，Codex worker 检测必须匹配这种真实 ID，否则会错误落到普通 OpenAI-compatible 单模型健康策略。
- 2026-05-14 第三阶段 worker 稳定性修复：流式 bootstrap 的 first activity 不能用“任意非空 payload”判断。`message_start`、普通 `content_block_start`、OpenAI role-only chunk、`[DONE]` 只是协议壳，不代表客户已收到真实内容；这些协议壳之后出现的 `empty_stream before first payload` 仍应允许请求内 retry/failover。只有真实 `content_block_delta`、文本/思考增量、tool_use / tool_calls / images 等内容性增量才算 first activity。
- 当前观察口径：缓存问题看 PG `usage_events.cached_tokens` 是否在同一 worker/auth 内随长会话增长，不能只看客户端 JSONL 的某一条旧 session；稳定性问题先区分 `context canceled` 客户端取消、Claude 直连上游错误、Codex worker 额度耗尽、Codex worker 空流。
