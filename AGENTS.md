...
- 2026-05-14 第二阶段 worker 稳定性修复：OpenAI-compatible worker 有时会以 HTTP 200 建立 SSE 后，在首个 `data:` 帧返回 `{"error":...}` / `{"type":"error","error":...}`。主程序必须把这类 SSE 错误帧转换为 executor `StreamChunk.Err`，不能当成普通流内容透传；否则 auth manager 无法触发请求内 failover、auth-level transient health 和 worker 冷却。普通 `chat.completion.chunk` 与 `[DONE]` 不应被误判。
