...
const (
	...
	probeCheckInterval     = 2 * time.Minute
	probeActivityWindow    = 15 * time.Minute
	probeRequestTimeout    = 120 * time.Second
	probeCanaryInterval    = 5 * time.Minute
	probeMaxConcurrency    = 4
	probeMaxTargetsPerTick = 8
)
...
const (
	probeKindHealth = "health"
	probeKindCanary = "canary"
)

const defaultProbeCanaryPrompt = "Explain in 3 short bullet points why exponential backoff improves reliability, then give 1 concise example."
...
type Result struct {
	...
	Error *Error
	// Probe marks results generated by the active provider health probe loop.
	Probe bool
	// ProbeKind distinguishes lightweight health probes from low-frequency canaries.
	ProbeKind string
}
...
type Manager struct {
	...
	probeMu       sync.Mutex
	probeInFlight map[string]struct{}
}
...
func NewManager(store Store, selector Selector, hook Hook) *Manager {
	...
	manager := &Manager{
		...
		probeSemaphore:   make(chan struct{}, probeMaxConcurrency),
...
func recordProviderProbeResult(state *ModelState, result Result, now time.Time) {
	if state == nil || !result.Probe {
		return
	}
	health := &state.Health
	completed := normalizeDuration(result.CompletedLatency)
	health.LastProbeAt = now
	health.LastProbeLatencyMs = completed.Milliseconds()
	probeSlow := completed >= providerHealthSlowCompletionThreshold
	health.LastProbeSlow = probeSlow
	if result.Success {
		health.LastProbeError = ""
	} else if result.Error != nil {
		health.LastProbeError = strings.TrimSpace(result.Error.Message)
	} else {
		health.LastProbeError = ""
	}
	if probeSlow {
		health.ConsecutiveSlowProbes++
	} else {
		health.ConsecutiveSlowProbes = 0
	}
	if result.ProbeKind == probeKindCanary {
		health.LastCanaryAt = now
		health.LastCanaryLatencyMs = completed.Milliseconds()
		health.LastCanarySlow = probeSlow
		if result.Success {
			health.LastCanaryError = ""
		} else if result.Error != nil {
			health.LastCanaryError = strings.TrimSpace(result.Error.Message)
		} else {
			health.LastCanaryError = ""
		}
		if probeSlow {
			health.ConsecutiveSlowCanaries++
		} else {
			health.ConsecutiveSlowCanaries = 0
		}
	}
}
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		if result.Success {
			if result.Model != "" {
				...
				recordProviderProbeResult(state, result, now)
			...
			if result.Model != "" {
				...
				recordProviderProbeResult(state, result, now)
				updateAggregatedAvailability(auth, now)
				if result.Probe && providerEvent == nil && providerAlertAppliesToError(auth, result.Error) {
					alertCtx := providerAlertContextFromAuth(auth, result.Model, result, now)
					kind := "probe_failure"
					if result.ProbeKind == probeKindCanary {
						kind = "canary_failure"
					}
					event := providerAlertEventFromContext(ctx, kind, alertCtx, result.Error)
					providerEvent = &event
				}
			} else {
...
// StartProviderHealthProbing launches a background loop that actively probes
// recently used or high-priority API-key providers for slow responses.
func (m *Manager) StartProviderHealthProbing(parent context.Context, interval time.Duration) {
	if m == nil {
		return
	}
	if interval <= 0 {
	...
	go func() {
		ticker := time.NewTicker(interval)
		defer ticker.Stop()
		m.checkProviderHealthProbes(ctx)
		for {
			select {
			case <-ctx.Done():
				return
...
// StopProviderHealthProbing cancels the background provider probe loop, if running.
func (m *Manager) StopProviderHealthProbing() {
	if m == nil {
		return
	}
	if m.probeCancel != nil {
...
type providerHealthProbeCandidate struct {
	Auth       *Auth
	Priority   int
...
func (m *Manager) checkProviderHealthProbes(ctx context.Context) {
	if m == nil {
		return
	}
	now := time.Now()
	snapshot := m.snapshotAuths()
	candidates := make([]providerHealthProbeCandidate, 0, len(snapshot))
	for _, auth := range snapshot {
		if auth == nil || !providerHealthProbeSupported(auth) || auth.Disabled {
			continue
		}
		activityAt := providerHealthActivityAt(auth)
		degraded := providerHealthIsDegraded(auth, now)
		priority := providerHealthPriority(auth)
		recentlyActive := !activityAt.IsZero() && now.Sub(activityAt) <= probeActivityWindow
		if !degraded && !recentlyActive && priority <= 0 {
			continue
		}
		if model := providerHealthProbeModel(auth); model == "" {
			continue
		}
		candidates = append(candidates, providerHealthProbeCandidate{
			Auth:       auth,
			Priority:   priority,
	...
	sort.SliceStable(candidates, func(i, j int) bool {
		...
		if !candidates[i].ActivityAt.Equal(candidates[j].ActivityAt) {
			return candidates[i].ActivityAt.After(candidates[j].ActivityAt)
		}
		return strings.Compare(candidates[i].Auth.ID, candidates[j].Auth.ID) < 0
	})

	limit := probeMaxTargetsPerTick
	if len(candidates) < limit {
		limit = len(candidates)
	}
	for i := 0; i < limit; i++ {
		auth := candidates[i].Auth
		if auth == nil || !m.markProbePending(auth.ID) {
			continue
		}
		go m.probeAuthWithLimit(ctx, auth.ID)
	}
}

func providerHealthProbeSupported(auth *Auth) bool {
	if !providerHealthPolicyApplies(auth) || auth == nil {
		return false
	}
	if mode := providerHealthProbeMode(auth); mode != "" && mode != "auto" {
		return true
	}
...
func providerHealthPriority(auth *Auth) int {
	if auth == nil {
		return 0
	}
	if auth.Attributes != nil {
...
func providerHealthActivityAt(auth *Auth) time.Time {
	if auth == nil {
		return time.Time{}
	}
	activityAt := auth.UpdatedAt
	for _, state := range auth.ModelStates {
		if state == nil {
			continue
		}
		if state.UpdatedAt.After(activityAt) {
			activityAt = state.UpdatedAt
		}
		if state.Health.LastProbeAt.After(activityAt) {
			activityAt = state.Health.LastProbeAt
		}
		if state.Health.LastCanaryAt.After(activityAt) {
			activityAt = state.Health.LastCanaryAt
		}
		if state.Health.LastSwitchAt.After(activityAt) {
			activityAt = state.Health.LastSwitchAt
		}
	}
...
func providerHealthIsDegraded(auth *Auth, now time.Time) bool {
	if auth == nil {
		return false
	}
	for _, state := range auth.ModelStates {
...
func providerHealthProbeModel(auth *Auth) string {
	if auth == nil {
		return ""
	}
	var degradedModel string
	var degradedUpdated time.Time
	var recentModel string
	var recentUpdated time.Time
	for model, state := range auth.ModelStates {
		if state == nil {
			continue
		}
		if wasProviderDegraded(state) {
			if state.UpdatedAt.After(degradedUpdated) || degradedModel == "" {
				degradedModel = model
				degradedUpdated = state.UpdatedAt
			}
		}
		if state.UpdatedAt.After(recentUpdated) || recentModel == "" {
			recentModel = model
			recentUpdated = state.UpdatedAt
		}
	}
	if degradedModel != "" {
...
type providerHealthProbePlan struct {
	Kind      string
	Mode      string
	Path      string
	Prompt    string
...
func providerHealthCanaryEnabled(auth *Auth) bool {
	if auth == nil || auth.Attributes == nil {
		return true
	}
	if raw := strings.TrimSpace(auth.Attributes["canary_enabled"]); raw != "" {
...
func providerHealthCanaryInterval(auth *Auth) time.Duration {
	if auth != nil && auth.Attributes != nil {
		if raw := strings.TrimSpace(auth.Attributes["canary_interval_seconds"]); raw != "" {
			if seconds, err := strconv.Atoi(raw); err == nil && seconds > 0 {
				return time.Duration(seconds) * time.Second
			}
		}
	}
	return probeCanaryInterval
}

func providerHealthCanaryPrompt(auth *Auth) string {
	if auth != nil && auth.Attributes != nil {
		if prompt := strings.TrimSpace(auth.Attributes["canary_prompt"]); prompt != "" {
			return prompt
		}
	}
	return defaultProbeCanaryPrompt
}

func providerHealthProbeMode(auth *Auth) string {
	if auth != nil && auth.Attributes != nil {
		if mode := strings.TrimSpace(auth.Attributes["probe_mode"]); mode != "" {
			return strings.ToLower(mode)
		}
	}
	switch {
	case auth != nil && isOpenAICompatAPIKeyAuth(auth):
		return "openai_chat"
	case auth != nil && strings.EqualFold(strings.TrimSpace(auth.Provider), "claude"):
		return "claude_messages"
	case auth != nil && strings.EqualFold(strings.TrimSpace(auth.Provider), "gemini"):
		return "gemini_generate_content"
	case auth != nil && strings.EqualFold(strings.TrimSpace(auth.Provider), "codex"):
...
func providerHealthProbePath(auth *Auth) string {
	if auth == nil || auth.Attributes == nil {
		return ""
	}
	return strings.TrimSpace(auth.Attributes["probe_path"])
}

func providerHealthProbePlanForAuth(auth *Auth, model string, now time.Time) providerHealthProbePlan {
	plan := providerHealthProbePlan{
	...
	if auth == nil || !providerHealthCanaryEnabled(auth) {
		return plan
	}
	interval := providerHealthCanaryInterval(auth)
	if interval <= 0 {
		return plan
	}
	state := auth.ModelStates[strings.TrimSpace(model)]
	if state == nil || state.Health.LastCanaryAt.IsZero() || now.Sub(state.Health.LastCanaryAt) >= interval {
		plan.Kind = probeKindCanary
		plan.Prompt = providerHealthCanaryPrompt(auth)
		plan.MaxTokens = 96
	}
	return plan
}

func (m *Manager) markProbePending(authID string) bool {
	if m == nil {
		return false
	}
	authID = strings.TrimSpace(authID)
	...
	if _, exists := m.probeInFlight[authID]; exists {
		return false
	}
	m.probeInFlight[authID] = struct{}{}
	return true
}

func (m *Manager) finishProbe(authID string) {
	if m == nil {
		return
	}
	authID = strings.TrimSpace(authID)
	...
	m.probeMu.Lock()
	delete(m.probeInFlight, authID)
	m.probeMu.Unlock()
}

func (m *Manager) probeAuthWithLimit(ctx context.Context, authID string) {
	defer m.finishProbe(authID)
	if m.probeSemaphore == nil {
		m.probeAuth(ctx, authID)
		return
	}
	select {
	case m.probeSemaphore <- struct{}{}:
		defer func() { <-m.probeSemaphore }()
	case <-ctx.Done():
		return
	}
	m.probeAuth(ctx, authID)
...
func (m *Manager) probeAuth(parent context.Context, authID string) {
	if m == nil {
		return
	}
	auth, ok := m.GetByID(authID)
	...
	model := providerHealthProbeModel(auth)
	executor := m.probeExecutorForAuth(auth)
	if model == "" || executor == nil {
		return
	}
	plan := providerHealthProbePlanForAuth(auth, model, time.Now())

	ctx, cancel := context.WithTimeout(parent, probeRequestTimeout)
	defer cancel()

	startedAt := time.Now()
	httpStatus, retryAfter, err := m.executeProviderHealthProbe(ctx, auth, executor, model, plan)
	completed := normalizeDuration(time.Since(startedAt))
	if errors.Is(err, errProviderHealthProbeUnsupported) {
		return
	}
	...
	result := Result{
		...
		CompletedLatency: completed,
		Probe:            true,
		ProbeKind:        plan.Kind,
	}
	if err != nil {
		result.Error = &Error{
			Code:       "provider_" + plan.Kind + "_failed",
			Message:    err.Error(),
...
var errProviderHealthProbeUnsupported = errors.New("provider health probe unsupported")

func (m *Manager) probeExecutorForAuth(auth *Auth) ProviderExecutor {
	if m == nil || auth == nil {
		return nil
	}
	candidates := []string{
	...
	if auth.Attributes != nil {
		candidates = append(candidates,
			strings.ToLower(strings.TrimSpace(auth.Attributes["provider_key"])),
			strings.ToLower(strings.TrimSpace(auth.Attributes["compat_name"])),
		)
	}
	...
	for _, candidate := range candidates {
		...
		if executor, ok := m.Executor(candidate); ok {
			return executor
		}
	}
	return nil
...
func (m *Manager) executeProviderHealthProbe(ctx context.Context, auth *Auth, executor ProviderExecutor, model string, plan providerHealthProbePlan) (int, *time.Duration, error) {
	if auth == nil || executor == nil {
		return 0, nil, errProviderHealthProbeUnsupported
	}
	body, requestURL, err := buildProviderHealthProbeRequest(auth, model, plan)
	if err != nil {
		return 0, nil, err
	}
	...
	if err != nil {
		...
		return statusCodeFromError(err), retryAfterFromError(err), err
	}
	defer resp.Body.Close()

	if resp.StatusCode >= 200 && resp.StatusCode < 300 {
		_, _ = io.Copy(io.Discard, io.LimitReader(resp.Body, 4096))
		return resp.StatusCode, parseHTTPRetryAfter(resp.Header.Get("Retry-After")), nil
	}
	...
	if resp.StatusCode == http.StatusBadRequest || resp.StatusCode == http.StatusNotFound || resp.StatusCode == http.StatusMethodNotAllowed || resp.StatusCode == http.StatusUnprocessableEntity {
		return resp.StatusCode, nil, errProviderHealthProbeUnsupported
	}
	return resp.StatusCode, parseHTTPRetryAfter(resp.Header.Get("Retry-After")), errors.New(providerHealthProbeErrorMessage(resp.StatusCode, data))
}

func buildProviderHealthProbeRequest(auth *Auth, model string, plan providerHealthProbePlan) ([]byte, string, error) {
	model = strings.TrimSpace(model)
	if auth == nil || model == "" {
		return nil, "", errProviderHealthProbeUnsupported
	}
	baseURL := strings.TrimRight(providerIdentityForAuth(auth).BaseURL, "/")
	if baseURL == "" {
		return nil, "", errProviderHealthProbeUnsupported
	}
	...
	case "openai_chat":
		...
		return body, resolveProviderHealthProbeURL(baseURL, plan.Path, "/chat/completions"), nil
	case "openai_responses", "codex_responses":
		...
		return body, resolveProviderHealthProbeURL(baseURL, plan.Path, "/responses"), nil
	case "claude_messages":
		...
		return body, resolveProviderHealthProbeURL(baseURL, plan.Path, "/v1/messages?beta=true"), nil
	case "gemini_generate_content":
		...
		return body, resolveProviderHealthProbeURL(baseURL, plan.Path, "/v1beta/models/"+url.PathEscape(model)+":generateContent"), nil
	default:
		return nil, "", errProviderHealthProbeUnsupported
	}
}

func resolveProviderHealthProbeURL(baseURL, overridePath, defaultPath string) string {
	...
	if strings.HasPrefix(overridePath, "/") {
		if parsed, err := url.Parse(baseURL); err == nil && parsed.Scheme != "" && parsed.Host != "" {
			parsed.Path = overridePath
			parsed.RawQuery = ""
			return parsed.String()
		}
	}
	return baseURL + "/" + strings.TrimLeft(overridePath, "/")
}

func applyProviderHealthProbeHeaders(req *http.Request, auth *Auth, plan providerHealthProbePlan) {
	if req == nil {
		return
	}
	...
	switch {
	case mode == "openai_chat" || mode == "openai_responses":
		req.Header.Set("User-Agent", "cli-proxy-health-probe")
	case mode == "claude_messages":
	...
	case mode == "codex_responses":
		...
		req.Header.Set("User-Agent", "cli-proxy-health-probe/0.101.0")
	}
}

func parseHTTPRetryAfter(raw string) *time.Duration {
...
func providerHealthProbeErrorMessage(statusCode int, body []byte) string {
	...
	return trimErrorMessage(fmtStatusCode(statusCode), message)
}
...
func trimErrorMessage(prefix, message string) string {
