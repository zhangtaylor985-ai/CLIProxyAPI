...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		if pendingAlert != nil && pendingAlert.AuthID != auth.ID {
			...
			targetIdentity := providerIdentityForAuth(auth)
			pendingAlert.SwitchedToMaskedAPIKey = targetIdentity.MaskedAPIKey
			pendingAlert.SwitchedToBaseURL = targetIdentity.BaseURL
			m.recordProviderSwitch(pendingAlert.AuthID, routeModel, auth.ID)
...
func authMatchesCodexChannelMode(auth *Auth, mode string) bool {
	...
	case "provider":
		...
		if authKind == "" && auth.Attributes != nil && strings.TrimSpace(auth.Attributes["api_key"]) != "" {
			return true
		}
		return false
	case "auth_file":
...
func (m *Manager) providerAlertContext(authID, model string, result Result) providerAlertContext {
	...
	return providerAlertContextFromAuth(auth, model, result, time.Now())
}
...
func providerIdentityForAuth(auth *Auth) providerIdentity {
	if auth == nil {
	...
	if identity.BaseURL == "" {
		switch strings.ToLower(strings.TrimSpace(providerAlertProviderName(auth))) {
		case "claude":
			identity.BaseURL = "https://api.anthropic.com"
		case "gemini":
			identity.BaseURL = "https://generativelanguage.googleapis.com"
		case "codex":
			identity.BaseURL = "https://chatgpt.com/backend-api/codex"
		}
	}
	return identity
}
...
func providerAlertEventFromContext(ctx context.Context, kind string, alertCtx providerAlertContext, err error) alerting.ProviderEvent {
	event := alerting.ProviderEvent{
		...
		MaskedAPIKey:         alertCtx.MaskedAPIKey,
		BaseURL:              alertCtx.BaseURL,
		Model:                alertCtx.Model,
		RequestID:            logging.GetRequestID(ctx),
		Cooldown:             alertCtx.Cooldown,
		CooldownUntil:        alertCtx.CooldownUntil,
...
func providerAlertContextFromAuth(auth *Auth, model string, result Result, now time.Time) providerAlertContext {
	ctx := providerAlertContext{
		Model:                strings.TrimSpace(model),
		FirstActivityLatency: normalizeDuration(result.FirstActivityLatency),
		CompletedLatency:     normalizeDuration(result.CompletedLatency),
	...
	ctx.AuthID = strings.TrimSpace(auth.ID)
	...
	ctx.MaskedAPIKey = identity.MaskedAPIKey
	ctx.BaseURL = identity.BaseURL
	if ctx.Model == "" {
	...
	if modelState := auth.ModelStates[ctx.Model]; modelState != nil && !modelState.NextRetryAfter.IsZero() {
		if wait := modelState.NextRetryAfter.Sub(now); wait > 0 {
			ctx.Cooldown = normalizeDuration(wait)
			ctx.CooldownUntil = modelState.NextRetryAfter
		}
	}
	if ctx.Cooldown <= 0 && !auth.NextRetryAfter.IsZero() {
		if wait := auth.NextRetryAfter.Sub(now); wait > 0 {
			ctx.Cooldown = normalizeDuration(wait)
			ctx.CooldownUntil = auth.NextRetryAfter
		}
	}
	return ctx
}

func recordProviderSwitch(source *Auth, model string, target *Auth, now time.Time) {
	...
	state := ensureModelState(source, model)
	if state == nil {
		return
	}
	health := &state.Health
	health.LastSwitchAt = now
	health.LastSwitchToProvider = strings.TrimSpace(providerAlertProviderName(target))
	health.LastSwitchToAuthID = strings.TrimSpace(target.ID)
	health.LastSwitchToAuthIndex = strings.TrimSpace(target.EnsureIndex())
	state.UpdatedAt = now
	source.UpdatedAt = now
}

func (m *Manager) recordProviderSwitch(sourceAuthID, model, targetAuthID string) {
	if m == nil {
		return
	}
	...
	model = strings.TrimSpace(model)
	if sourceAuthID == "" || targetAuthID == "" || model == "" {
		return
	}
	m.mu.Lock()
	defer m.mu.Unlock()
	source := m.auths[sourceAuthID]
	target := m.auths[targetAuthID]
	recordProviderSwitch(source, model, target, time.Now())
}
				...
				if degraded {
					...
					if !state.NextRetryAfter.IsZero() {
						if wait := state.NextRetryAfter.Sub(now); wait > 0 {
							...
							alertCtx.CooldownUntil = state.NextRetryAfter
						}
					}
