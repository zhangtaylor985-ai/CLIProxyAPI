...
- 默认长期链路走 `archive -> handoff -> import -> export`
- 默认常驻脚本是 `scripts/archive_handoff_loop.py`
- 游标与状态文件统一使用 `UTC` / RFC3339 `Z` 时间
- `live_export_delete_loop.py` 保留为人工排障备用，不再作为默认路径

## Live Export 是什么

`live export` 指的是：

- 直接从 PostgreSQL 按筛选条件导出需求方最终目录格式
- 不经过 raw archive 文件
- 不经过本地 `import_session_trajectory_archive.py`

它适合：

- 用户已经明确只要最终目录 / JSON 交付物
- 不需要清理远端数据库
- 不需要走 `archive -> handoff -> import -> export` 全链路

它不负责：

- 清空远端 PG 的这批数据
- 在完成后自动接本地 `import/export`
- 替代 archive 归档链路

一句话：

- `live export` 是“直接出最终文件”
- `archive/handoff` 是“先归档减压，再自动接本地恢复与导出”

## Live Export + Verified Delete Loop
...
`scripts/live_export_delete_loop.py`

它做的是：

1. 冻结一批 `last_activity_at < now() - inactive_hours` 的冷会话快照
2. 直接从远端 PG 导出最终需求格式
3. 删除同一批仍然保持冷态的远端数据
4. 可选 `VACUUM`
5. 继续轮询下一批
...
python3 scripts/live_export_delete_loop.py \
  --mode status \
  --pg-dsn 'postgres://.../cliproxy?sslmode=require'
```
...
如果用户问 archive/handoff 长链路的实时状态，不要去看 live export watcher，优先使用：

`scripts/archive_handoff_loop.py --mode status`
...
如果用户希望从“启动 archive”开始就一条后台任务跑到底，优先改用：
...
更完整说明继续看总览文档，不在多个 skills 里重复展开。
